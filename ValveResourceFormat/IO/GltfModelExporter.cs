...
#pragma warning disable CS0168 // Variable is declared but never used
        ...
        private async Task GenerateGLTFMaterialFromRenderMaterial(Material material, VMaterial renderMaterial, ModelRoot model)
        {
            ...
            try
            {
                ...
                foreach (var (textureKey, texturePath) in renderMaterial.TextureParams)
                {
                    ...
                    var mainInstruction = textureKey switch
                    {
                        "g_tColor" => new RemapInstruction("BaseColor", ChannelMapping.RGBA, ChannelMapping.RGB),
                        "g_tNormal" => new RemapInstruction("Normal", ChannelMapping.RGBA, ChannelMapping.RGB),
                        "g_tAmbientOcclusion" => new RemapInstruction("Occlusion", ChannelMapping.R, ChannelMapping.R),
                        "g_tMetalness" => new RemapInstruction("MetallicRoughness", ChannelMapping.RGBA, ChannelMapping.RGB),
                        _ => null,
                    };
        ...
        private async Task LinkAndSaveImage(Image image, byte[] pngBytes)
        {
            ...
            fileName = fileName switch
            {
                ...
                _ when fileName.Contains("_ao") => fileName + "_512.png",
                _ when fileName.Contains("_color") => fileName + "_512.jpg",
                _ when fileName.Contains("_rough") => fileName + "_512.png",
