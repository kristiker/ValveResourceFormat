namespace ValveResourceFormat.Renderer
{
    ...
    /// </summary>
    /// <remarks>
    /// A tree earns its keep by amortizing build cost over many queries, which needs contents that
    /// mostly hold still. Every node in here moves every frame, so an octree paid a descent, a linear
    /// scan of a cell and a list removal per node per frame to accelerate a handful of frustum passes
    /// over a set small enough to brute force in microseconds. Bounds are kept as center and extent
    /// because that is the form the frustum test wants.
    /// </remarks>
    public class DynamicNodeSet
    {
        ...
        public DynamicNodeSetDebugRenderer? DebugRenderer { get; set; }
        ...
        /// </summary>
        /// <param name="node">The node whose bounds changed.</param>
        ...
        /// <param name="results">The list to append visible nodes to.</param>
        ...
        /// </summary>
        /// <param name="boundingBox">The box to test against.</param>
        /// <param name="results">The list to append intersecting nodes to.</param>
        public void Query(in AABB boundingBox, List<SceneNode> results)
        {
            ...
            for (var i = 0; i < Count; i++)
            {
                if (boundingBox.Intersects(GetBounds(i)))
                {
                    results.Add(nodes[i]);
                }
            }
