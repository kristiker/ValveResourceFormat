...
public sealed class SoundEventPlayer : IDisposable
{
    ...
    /// <summary>
    /// Registers the scene's soundscapes and starts its <c>point_soundevent</c> entities that have
    /// "Start On Spawn" set. The rest require entity I/O, which the viewer does not simulate.
    /// </summary>
    /// <param name="scene">The loaded scene whose entities are scanned.</param>
    public void StartMapSoundEvents(Scene scene)
    {
        ...
        foreach (var entityData in pointSoundEvents)
        {
            ...
            Play(soundName, GetSoundEventPosition(entityData, namedEntities));
        }
    }
