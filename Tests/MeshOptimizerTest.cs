...
namespace Tests
{
    public partial class MeshOptimizerTest
    {
        ...
        [Test]
        public async Task DecodesBasicMeshlet()
        {
            const int vertexCount = 7;
            const int triangleCount = 5;

            var vertices = new byte[vertexCount * sizeof(uint)];
            var triangles = new byte[triangleCount * 3 + 1]; // the reference test sizes the buffer one over
        ...
        [Test]
        public async Task DecodesTypicalMeshlet()
        {
            const int vertexCount = 32;
            const int triangleCount = 44;
